...
def make_mock_allocation_response(candidates):
    result = recommend_assignee(candidates)
...
def get_reassignment_suggestion(project_data, exclude_person=None):
    ...
    for member in project_data.get("team", []):
        ...
        candidates.append({"name": member["name"], "utilization_pct": round(pct)})
    ...
    use_mock = os.getenv("USE_MOCK_AGENT", "0") == "1"
    if use_mock:
        response = make_mock_allocation_response(candidates)
    else:
        prompt = f"Candidates: {candidates}. Who should take a new task?"
