...
def make_mock_budget_response(budget):
    ...
    finding = "over_budget" if result.startswith("over_budget") else "on_budget"
    llm_text = f"Finding: {finding}\nConfidence: 1.0\nReason: Generated by mock local stub."
    return {
        "messages": [
...
def run_budget_agent(project_data):
    ...
    findings.append({
        ...
        "finding": finding,
        "raw_tool_outputs": tool_outputs,
        "user_response": f"[{'OVER BUDGET' if finding == 'over_budget' else 'ON BUDGET'}] Project spend: {result}"
    })
    return findings
...
def resolve_conflicts(all_findings, project_data):
    ...
    is_over_budget = any(f["finding"] == "over_budget" for f in budget_findings)
    ...
    for rf in risk_findings:
        ...
        overloaded = assignee and assignee in resource_findings and resource_findings[assignee]["finding"] == "overloaded"
