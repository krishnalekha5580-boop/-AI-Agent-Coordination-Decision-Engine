def check_budget_status(planned_spend: float, actual_spend: float, pct_time_elapsed: float) -> str:
    ...
    try:
        ...
        if pct_spent > pct_time_elapsed + 15:
            return f"over_budget: {pct_spent:.0f}% spent vs {pct_time_elapsed:.0f}% time elapsed"
        return f"on_budget: {pct_spent:.0f}% spent vs {pct_time_elapsed:.0f}% time elapsed"
